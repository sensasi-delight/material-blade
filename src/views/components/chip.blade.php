...
<div {{ $attributesPreprocess($attributes->class(['mdc-chip--deletable' => $variant === 'input' && !$isDisabled])) }}>
    ...
    <span role="gridcell">
        <span role="{{$variant === 'filter' ? 'checkbox' : 'button'}}" tabindex="0" class="mdc-chip__primary-action"@if($variant === 'filter') aria-checked="{{ $isSelected }}"@endif>
    ...
    @if ($variant === 'input' && !$isDisabled)
        <span role="gridcell">
            <i class="material-icons mdc-chip__icon mdc-chip__icon--trailing mdc-chip__action--trailing" tabindex="-1" role="button">cancel</i>
        </span>
    @endif
</div>
